...
Future<Token> getToken(String username, String password) async {
  var loginRes = await http.post('https://www.easistent.com/p/ajax_prijava',
      body: {'uporabnik': username, 'geslo': password});
  var cookie = loginRes.headers['set-cookie'].split(';')[0];

  var tokenRes =
      await http.get('https://www.easistent.com', headers: {'cookie': cookie});
  var document = parse(tokenRes.body);

  try {
    String childId = document.head
        .querySelector('meta[name="x-child-id"]')
        .attributes['content'];
    String bearer = document.head
